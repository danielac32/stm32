...
int load_elf( program_img *prg, exec_img *res) {
    ...
    /*const */ uintptr_t to_addr = (uintptr_t)syscall_malloc(ALIGN(4, size));
    if(to_addr==NULL){
        panic("elf ->> error malloc\n");
    }
...
int load_elf2( void *prg,char *out) {
    // note: I am assuming my binary starts at 0x0 throughout this function
    ...
    /*const */ void* to_addr = (void *)syscall_malloc(ALIGN(4, size));
    if(to_addr==NULL){
        panic("elf ->> error malloc\n");
    }
    //res->start = (void*)to_addr; 
    ...
    if ((fptr = syscall_fopen(out,"w+")) == NULL){
       panic("Error! opening file");
       return -1;
    }


    syscall_fwrite(to_addr, size, 1, fptr); 
    syscall_fclose(fptr);
    syscall_free(to_addr);
    return 0;
}
