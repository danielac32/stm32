...
#define	PORT_SCK        GPIO_B   
#define	PIN_SCK         13
#define	PORT_DC       GPIO_A    
#define	PIN_DC        10
#define	PORT_MISO       GPIO_B    
#define	PIN_MISO       14
#define	PORT_MOSI       GPIO_B    
#define	PIN_MOSI        15
#define	PORT_CS         GPIO_A   // PB14 --> F_CS(0)
#define	PIN_CS          8
#define	PORT_RESET         GPIO_B   // PB14 --> F_CS(0)
#define	PIN_RESET          12
...
#else
void transfer(unsigned char n_dato){
 
   unsigned char caa;
   hw_set_pin(GPIOx(PORT_SCK), PIN_SCK, 0);//gpio_set(_SCK,0);  //nok_sclk=0;
   for (caa=8;caa;caa--){      //MSB first
      if (n_dato & 0x80){
          hw_set_pin(GPIOx(PORT_MOSI), PIN_MOSI, 1);//gpio_set(_SDO,1);//nok_sda=1;
      }else{
          hw_set_pin(GPIOx(PORT_MOSI), PIN_MOSI, 0);//gpio_set(_SDO,0);//nok_sda=0;
      }
      n_dato = n_dato << 1;

      hw_set_pin(GPIOx(PORT_SCK), PIN_SCK, 1);//gpio_set(_SCK,1);//nok_sclk=1;
      hw_set_pin(GPIOx(PORT_SCK), PIN_SCK, 0);//gpio_set(_SCK,0);// nok_sclk=0;
   }
}
#endif
void initSPI()
{
	...
	hw_cfg_pin(GPIOx(PORT_SCK),    PIN_SCK,    GPIOCFG_MODE_ALT | GPIO_AF5_SPI2 | GPIOCFG_OSPEED_VHIGH | GPIOCFG_OTYPE_PUPD | GPIOCFG_PUPD_PUP);//hw_cfg_pin(GPIOx(PORT_SCK),    PIN_SCK,   GPIOCFG_MODE_OUT | GPIOCFG_OSPEED_VHIGH  | GPIOCFG_OTYPE_PUPD | GPIOCFG_PUPD_PUP);
	hw_cfg_pin(GPIOx(PORT_DC),   PIN_DC,   GPIOCFG_MODE_OUT | GPIOCFG_OSPEED_VHIGH  | GPIOCFG_OTYPE_PUPD | GPIOCFG_PUPD_PUP);
	hw_cfg_pin(GPIOx(PORT_MOSI),   PIN_MOSI,   GPIOCFG_MODE_ALT | GPIO_AF5_SPI2 | GPIOCFG_OSPEED_VHIGH | GPIOCFG_OTYPE_PUPD | GPIOCFG_PUPD_PUP);//hw_cfg_pin(GPIOx(PORT_MOSI),   PIN_MOSI,   GPIOCFG_MODE_OUT | GPIOCFG_OSPEED_VHIGH  | GPIOCFG_OTYPE_PUPD | GPIOCFG_PUPD_PUP);
    ...
    hw_set_pin(GPIOx(PORT_RESET), PIN_RESET, 1);

   	SPI2->CR1 = SPI_CR1_SSM | SPI_CR1_SSI | SPI_CR1_SPE | SPI_CLK_DIV | SPI_CR1_MSTR;
...
void ST7735_Reset()
{
		...
		hw_set_pin(GPIOx(PORT_RESET), PIN_RESET, 0);
		delay(5);
		hw_set_pin(GPIOx(PORT_RESET), PIN_RESET, 1);
		//gpio_put(st7735_pinRST, 1);
...
void ST7735_Select()
{
	hw_set_pin(GPIOx(PORT_CS), PIN_CS, 0);//gpio_put(st7735_pinCS, 0);
}

void ST7735_DeSelect()
{
	hw_set_pin(GPIOx(PORT_CS), PIN_CS, 1);//gpio_put(st7735_pinCS, 1);
}

void ST7735_RegCommand()
{
	hw_set_pin(GPIOx(PORT_DC), PIN_DC, 0);//gpio_put(st7735_pinDC, 0);
}

void ST7735_RegData()
{
	hw_set_pin(GPIOx(PORT_DC), PIN_DC, 1);//gpio_put(st7735_pinDC, 1);
}
