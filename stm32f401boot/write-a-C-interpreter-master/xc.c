...
enum { LEA ,IMM ,JMP ,CALL,JZ  ,JNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PUSH,
       ...
       OPEN,READ,CLOS,PRTF,MALC,MSET,MCMP,EXIT,SYSCALL };
            ...
            if (assembly) {
                ...
                while (old_text < text) {
                    printf("%8.4s", & "LEA ,IMM ,JMP ,CALL,JZ  ,JNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PUSH,"
                                      "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
                                      "OPEN,READ,CLOS,PRTF,MALC,MSET,MCMP,EXIT,SYSCALL"[*++old_text * 5]);
...
int eval() {
    ...
    while (1) {
        ...
        else if (op == SYSCALL) { ax = 0; printf("(%d)\n", *sp);}
        else {
...
int main(int argc, char **argv)
{
    ...
    printf("size %d\n",poolsize );
    //poolsize = 10000;//256 * 1024; // arbitrary size
    line = 1;
    ...
    if (!(stack = malloc(2000))) {
        printf("could not malloc(%d) for stack area\n", poolsize);
        return -1;
    }
    if (!(symbols = malloc(5000))) {
   ...
   // memset(symbols, 0, poolsize);
    ...
    src = "char else enum if int return sizeof while "
          "open read close printf malloc memset memcmp exit syscall void main";
    ...
    int res=fread(src,poolsize,1, fptr);
