...
static void print(char *str){
    ...
    //while(usbd_ep_write(&udev, CDC_TXD_EP, str, strlen(str))==-1);
...
int lib_putc2(int ch)
{
    ...
    //while(usbd_ep_write(&udev, CDC_TXD_EP, &ch, 1)==-1);
    while(1){
        int len = 0;//usbd_ep_write(&udev, CDC_TXD_EP, &ch, 1);
        if (len < 0){
            continue;
        }else break;
    }
