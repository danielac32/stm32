...
extern EEPROM_Status EEPROM_WaitForLastOperation(uint32_t Timeout);
extern EEPROM_Status EEPROM_Write(uint32_t Address, uint32_t Data);
extern uint32_t EEPROM_Read(uint32_t address);

extern void ReadBuffer_EEPROM(uint32_t addr, volatile uint32_t *buffer, uint32_t len);
extern void SaveBuffer_EEPROM(uint32_t addr, volatile uint32_t *buffer, uint32_t len);

extern void unlock_flash();
extern void lock_flash();
extern void write_flash(uint32_t addr, uint32_t data);
extern uint32_t read_flash(uint32_t Address);
extern void erase_flash_sector(uint32_t sector);
