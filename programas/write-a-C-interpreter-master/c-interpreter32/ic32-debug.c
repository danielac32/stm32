...
// instructions
enum { LEA =0,IMM ,JMP ,CALL,JZ  ,JNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PUSH,
       OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,
       OPEN,READ,CLOS,PRTF,MALC,MSET,MCMP,EXIT,LABEL};

#ifdef SUPPORT_DEBUG
const char *inst_str[] =
{
  "LEA","IMM","JMP","CALL","JZ ","JNZ","ENT","ADJ","LEV","LI ","LC ","SI ","SC ","PUSH"," OR ","XOR","AND","EQ ","NE ","LT ","GT ","LE ","GE ","SHL","SHR","ADD","SUB","MUL","DIV","MOD"," OPEN","READ","CLOS","PRTF","MALC","MSET","MCMP","EXIT","LABEL",
};
...
/*#ifdef SUPPORT_DEBUG
  ...
  debug_input:
        ...
        //else if (op == RET)  {pc = (int *)*sp++;}                              // return from subroutine;
            ...
            printf("exit(%d)", *sp); 
            //hexDump ("data", data, poolsize);
            ///hexDump ("stack", stack, poolsize);
            return *sp;
        }
       // else if (op == LABEL) { ax = 0; }
...
int main(int argc, char **argv)
{
    ...
    src = "char else enum if int return sizeof while "
          "open read close printf malloc memset memcmp exit void main label";
